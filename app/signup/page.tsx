...
import { Play, User as UserIcon, Mail, Phone, Lock, ArrowRight, CheckCircle2 } from 'lucide-react';
...
function SignupForm() {
  ...
  const { signUpWithPassword, signUpWithPhone, loginWithGoogle } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let res: { success: boolean; error?: string; alreadyExists?: boolean; needsEmailVerification?: boolean };

    if (authMethod === 'phone') {
      res = await signUpWithPhone(phone, password, name);
    } else {
      res = await signUpWithPassword(email, password, name);
    }
            ...
            <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl space-y-2">
              ...
              {alreadyExists && (
                <div className="pt-0.5">
                  <Link
                    href={`/login?mode=${authMethod}&redirectTo=${encodeURIComponent(redirectTo)}`}
              ...
              <div className="space-y-1">
                ...
                <div className="relative flex">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-zinc-50 text-zinc-900 placeholder-zinc-400 text-xs rounded-r-lg border border-zinc-200 focus:outline-none focus:border-orange-500 focus:bg-white font-medium transition-colors"
                  />
                </div>
              </div>
            )}
