...
import { Play, Mail, Phone, Lock, ArrowRight, CheckCircle2 } from 'lucide-react';
import { SAMPLE_VIDEOS } from '@/data/mockData';

function LoginForm() {
  const searchParams = useSearchParams();
  const initialMode = searchParams?.get('mode') === 'phone' ? 'phone' : 'email';
  const [authMethod, setAuthMethod] = useState<'email' | 'phone'>(initialMode);
  ...
  const { loginWithPassword, loginWithPhone, loginWithGoogle } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let res: { success: boolean; error?: string; notFound?: boolean };
    ...
    if (authMethod === 'phone') {
      ...
      res = await loginWithPhone(phone, password);
    } else {
        ...
        <div className="md:col-span-7 p-6 sm:p-8 flex flex-col justify-between space-y-5">
          <div>
            ...
            <p className="text-xs text-zinc-500 font-normal mt-0.5">
              Sign in to access your purchased video bundles and instant downloads.
            </p>
          </div>
          ...
          {registered && !error && (
            <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-medium rounded-lg flex items-center space-x-2">
            ...
            <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl space-y-2">
              ...
              {notFound && (
                <div className="pt-0.5">
                  <Link
                    href={`/signup?mode=${authMethod}&redirectTo=${encodeURIComponent(redirectTo)}`}
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            {authMethod === 'email' ? (
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3.5 py-2.5 bg-zinc-50 text-zinc-900 placeholder-zinc-400 text-xs rounded-lg border border-zinc-200 focus:outline-none focus:border-orange-500 focus:bg-white font-medium transition-colors"
                  />
                </div>
              </div>
            ) : (
              <div className="space-y-1">
                ...
                <div className="relative flex">
                  <div className="inline-flex items-center px-3 py-2 bg-zinc-100 border border-r-0 border-zinc-200 rounded-l-lg text-xs font-bold text-zinc-700 select-none">
                    🇮🇳 +91
                  </div>
                  <input
                    type="tel"
                    ...
                    className="w-full px-3 py-2.5 bg-zinc-50 text-zinc-900 placeholder-zinc-400 text-xs rounded-r-lg border border-zinc-200 focus:outline-none focus:border-orange-500 focus:bg-white font-medium transition-colors"
                  />
                </div>
              </div>
            )}
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-zinc-500 hover:text-zinc-800 font-medium cursor-pointer">
                  Forgot password?
                </span>
              </div>
              <div className="relative">
            ...
            <button
              ...
              className="w-full py-2.5 bg-orange-600 hover:bg-orange-500 text-white text-xs font-semibold rounded-lg shadow-xs transition-colors flex items-center justify-center space-x-1.5 disabled:opacity-50 cursor-pointer"
            >
              {isSubmitting ? (
            ...
            <div className="space-y-2">
              ...
              {authMethod === 'email' ? (
                <button
                  ...
                  disabled={isSubmitting}
                  className="w-full py-2.5 bg-zinc-50 hover:bg-zinc-100 text-zinc-700 text-xs font-semibold rounded-lg border border-zinc-200 shadow-xs transition-colors flex items-center justify-center space-x-2 disabled:opacity-50 cursor-pointer"
                >
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-[80vh] flex items-center justify-center text-xs text-zinc-500">Loading Login...</div>}>
      <LoginForm />
    </Suspense>
  );
}
